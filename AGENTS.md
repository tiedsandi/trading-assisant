...
Trading Assistant uses Next.js, a Go HTTP API, and PostgreSQL in Docker.
Authentication v1 is implemented; trading features are not yet implemented.
...
## Repository map
- `frontend/`: App Router in `src/app`, auth in `src/features/auth`, shadcn UI, `tests`, Bun manifests.
- `backend/`: `cmd/api`, `cmd/migrate`, `db/migrations`, `internal/modules/auth`, HTTP/config/database packages, integration tests.
...
Migration service runs before backend startup. After adding a migration to a running stack:
...
- Follow existing conventions. Database schema changes must use versioned Goose SQL migrations in `backend/db/migrations`.
